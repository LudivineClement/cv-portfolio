  ...
  return (
    <div className=" flex w-1/3 text-center h-screen overflow-hidden fixed left-0 ">
      ...
      <div className=" w-5/6 px-3 text-white flex flex-col justify-center bg-gradient-to-tr from-pink-300 via-purple-300 to-indigo-400">
        <div ><img src="/img/img-profile.png" alt="avatar" className="h-40 inline-block mx-auto mb-10" /></div>
        ...
        <button className="text-sm group uppercase max-w-[250px] mx-auto mt-20 tracking-widest will-change-transform rounded-full bg-color-dark border-2 border-transparent hover:bg-white hover:text-color-light hover:border-2 hover:scale-105 hover:border-color-light transition-all duration-200 ease-in text-white px-4 py-3">Télécharger le cv <BiDownload className='inline-block text-2xl mb-1 ml-1 text-color-dark bg-white group-hover:text-white group-hover:bg-color-light rounded-full p-1' /> </button>
      </div>
    </div>
  );
};
